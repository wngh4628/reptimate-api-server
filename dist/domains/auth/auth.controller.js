...
let AuthController = class AuthController {
    ...
    async login(req, res, dto) {
        const result = await this.authService.login(req.get('user-agent').toLowerCase(), dto);
        return http_response_1.default.created(res, { body: result });
    }
    async socialLogin(req, res, dto) {
        const result = await this.authService.socialLogin(req.get('user-agent').toLowerCase(), dto);
        return http_response_1.default.created(res, { body: result });
    }
    async getNewAccessToken(req, res, dto) {
        const result = await this.authService.getNewAccessToken(req.get('user-agent').toLowerCase(), dto.refreshToken);
        return http_response_1.default.created(res, { body: result });
    }
    async logout(req, res, user) {
        await this.authService.logout(req.get('user-agent').toLowerCase(), user.idx);
...
__decorate([
    ...
    __param(2, (0, common_1.Body)()),
    __metadata("design:type", Function),
    __metadata("design:paramtypes", [Object, Object, social_login_user_dto_1.SocialLoginUserDto]),
    __metadata("design:returntype", Promise)
...
__decorate([
    ...
    (0, api_created_response_1.ApiCreatedResponseTemplate)({ type: login_response_dto_1.getNewAccessTokenDto }),
